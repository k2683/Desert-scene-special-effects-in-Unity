Shader "PostEffect/VolumeLighting"
{
    ...
    SubShader
    {
        ...
        float _blurInt;
        half4 _LightColor0;
        float _B;//瑞利散射系数，本应与h，λ相关，这里为节省开销直接给一个常量
        float _WaveLength; //可见光波长
        float _H;//距离海平面高度
        float _Tcp;
        half4 _VolumColor;
       ...
       // TEXTURE2D(_TempTargetVolumLighting); SAMPLER(sampler_TempTargetVolumLighting);
        ...
        Pass
        {
            ...
            /*参考文章 https://zhuanlan.zhihu.com/p/360832044*/
            //https://blog.csdn.net/weixin_29315091/article/details/113331020

            //以下函数出于性能考虑皆是简化后的函数
            //密度函数
            float P(float h)//h代表当前散射点高度为零时代表海平面附近
            {
                float p = exp(-h/8500); //瑞利散射参考高度8500m
                return p;
            }
            //相位函数
            float R(float cos) //cos代表了光方向与视线方向的余弦值 ， unity中获取的为光方向的反方向这里注意
            {
                float r = (1 + cos * cos) * 0.0596831; //这里的0.0596831代表了3/（16Π），少让计算机算点是一点
                return r;
            }
            //光线衰减函数
            float T(float h , float dis)
            {
                float t = exp(-_B * P(h) * dis);
                return t;
            }
            ...
            half4 frag(Varyings i): SV_Target
            {
                ...
                float totalInt = 0;
                float d = 0;
                float cos = dot(lDirWS , rd);
                for(int j = 0; j < _MaxStep; j++)
                {
                    d += delta;
                    if(d > m_length) break;
                    currentPos += delta * rd;
                    totalInt += _LightIntensity * GetShadow(currentPos) * T(_H , d) * S(cos);
                }
                half3 lightCol = totalInt * _LightColor0.rgb * _VolumColor.rgb;
